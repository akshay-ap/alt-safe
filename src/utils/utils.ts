...
import safeModuleSetupABI from "../abis/SafeModuleSetup.json";
import {
  type SafeDeployment,
  SafeDeploymentType,
  SafeVersion,
...
function encodeSafeModuleSetupCall(modules: string[]): string {
  return encodeFunctionData({ abi: safeModuleSetupABI, functionName: "enableModules", args: [modules] });
}

/**
 * Calculates the initialization data for Safe contract deployment.
 *
 * This function generates the encoded calldata for the Safe's `setup` function,
 * which is used during Safe proxy deployment to initialize the Safe with the
 * specified configuration including owners, threshold, modules, and other settings.
 *
 * @param owners - Array of owner addresses that will control the Safe
 * @param threshold - Number of required signatures for transaction execution (must be <= owners.length)
 * @param fallbackHandler - Address of the fallback handler contract for handling unknown function calls
 * @param to - Optional setup modules contract address for enabling modules during initialization
 * @param modules - Optional array of module addresses to enable during Safe setup
 * @param _guard - Optional guard contract address (currently unused in implementation)
 * @param _moduleGuard - Optional module guard contract address (currently unused in implementation)
 *
 * @returns Encoded calldata for the Safe setup function as a hex string
 *
 * @remarks
 * - If modules are provided and `to` address is set, the function will encode a delegate call
 *   to enable the specified modules during Safe initialization
 * - The function uses zero addresses for gasToken and refundReceiver parameters
 * - Guard parameters are currently not implemented in the setup call
 */
export const calculateInitData = (
  owners: Address[],
  threshold: number,
  ...
  modules?: Address[],
  _guard?: Address,
  _moduleGuard?: Address,
) => {
  let delegateCallAddress: Address = zeroAddress;
  let dataBytes = "0x";
  if (to && to !== zeroAddress && modules && modules.length > 0) {
    // Encode the modules setup call
    dataBytes = encodeSafeModuleSetupCall(modules);
    delegateCallAddress = to;
  }
